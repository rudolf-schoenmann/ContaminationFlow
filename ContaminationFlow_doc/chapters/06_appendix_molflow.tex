...
\rule{0pt}{3ex} getAverageStatistics()& Calculate average ratio (std/mean) weighted with area\\
...
\rule{0pt}{3ex}  \multirow{2}{*}{counterWindowPercent}&  Percentage of step size (posterior)\\ & at which velocity counters are increased\\
\rule{0pt}{3ex}  desWindowPercent&  Percentage of step size (anterior) for desorption\\
...
\rule{0pt}{3ex} \multirow{2}{*}{UpdateErrorList()}& Calculates error per facet, see eq.\ \ref{eq:error}\\&Saves to \codew{simHistory}\\
\rule{0pt}{3ex} UpdateErrorSub()& UpdateErrorList()\\
\rule{0pt}{3ex} \multirow{2}{*}{UpdateErrorAll()}& Sums up total error of facets \& weights by facet area \\ & for all possible error quantities\\
\rule{0pt}{3ex} UpdateError()&  UpdateErrorAll() for only one error quantity in sub process\\
