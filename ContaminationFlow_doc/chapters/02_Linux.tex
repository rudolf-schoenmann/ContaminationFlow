...
%\item Update and accumulation of counters and other information such as profiles
...
%\draw [help lines] (0,0) grid (4,4);
	...
	{
		\textbf{Main Process 0}
		\nodepart{second} \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]\item Update contamination amount in hitbuffer\item Update SimulationHistory\end{itemize}
	};
...
\item \codew{$coveringPath$}: Path to covering file, contains either covering or coverage per facet, file can be exported from the Windows version of ContaminationFlow, e.g.\ \codew{\textasciitilde/covering.txt}; default: \codew{""}
...
\item \codew{$maxTime$}: Maximum simulated time; default: \codew{10.0}
\item \codew{$maxUnit$}: Maximum simulated time unit; default: \codew{y}
\item \codew{$iterationNumber$}: Number of iterations; default: \codew{100}
\item \codew{$usePCMethod$}: Use predictor-corrector method "1" or simulate without predictor-corrector method "0"; default: \codew{0}
...
\item \codew{$targetParticles$}: Minimum number of desorbed + outgassed test-particles per iteration; default: \codew{1000}
\item \codew{$targetError$}: Avg.\ statistical uncertainty (error) to be achieved for each iteration, calculated as the (by the facets area weighted) average of the normalized standard deviation of events per facet; default: \codew{0.001}
...
\item \codew{$counterWindowPercent$}: Percentage of step size (posterior fraction of the step size) at which pressure and density is sampled; default: \codew{0.1}
\item \codew{$rollingWindowSize$}: Number of iterations for average statistics; default: \codew{10}
\item \codew{$convergenceTarget$}: Target for average statistics to indicate convergence; default: \codew{1}
\item \codew{$stopConverged$}: Stop simulation if average statistics indicate convergence; default: \codew{1}
\item \codew{$convergenceTime$}: Minimum simulated time to stop simulation if converged; default: \codew{0}
...
\item \codew{$saveConsole$}: Determines if console output is additionally saved; default:\codew{0}
\end{itemize}
\bigskip
...
\item Subprocesses send updated Databuff struct containing hitbuffer and required SimulationHistory values to main process 0 using \codew{MPI\_Send($\cdot$)} and \codew{MPI\_Recv($\cdot$)}
\end{itemize}
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Either \codew{$E_{de}$} or \codew{$H_{vap}$}, depending on the coverage
...
\subsubsection{Statistical uncertainty ("error")}
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
...
\item Error calculated in \codew{UpdateErrorList($\cdot$)} for facet error \& in \codew{UpdateErrorAll($\cdot$)} for the average error weighted by the facets' areas (\codew{UpdateSubProcess.cpp})
\end{itemize}
...
\code{class SimulationHistory \{\\
	...
	\hphantom{\enskip}HistoryList<llong> outgassdedList;{\color{gray}//Number of outgassed particles}\\
	\hphantom{\enskip}HistoryList<llong> adsorbedList;{\color{gray}//Number of adsorbed particles}\\
	\hphantom{\enskip}HistoryList<double> errorList\_event;{\color{gray}//error event: hits, desorbed \& outgassed test-particles}\\
	\hphantom{\enskip}HistoryList<double> errorList\_covering;{\color{gray}//error covering: desorbed \& adsorbed test-particles}\\
...
\begin{tikzpicture}[node distance=2cm, font=\scriptsize]
	...
	\node[module](start)  at (1,-0.2) {Start simulation\\subprocess};
...
\end{tikzpicture}
\caption{Overview: simulation on subprocesses}
\end{figure}
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Multiply covering counter so that smallest covering $\geq$ \codew{ProblemDef::coveringThreshMin}
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Desorption in \codew{UpdateDesorption($\cdot$)}  (\codew{UpdateSubProcess.cpp})
...
\item Residence time randomly calculated from binding energy, facet temperature and oscialltion frequency in \codew{PerformBounce($\cdot$)}  (\codew{SimulationMC.cpp})
\end{itemize}

\subsubsection{Increase facet counters in case of desorb, absorb or hit}
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Increase hit, desorbed, outgassed or absorbed counter according to event
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Calculate statistical uncertainty 'error' in \codew{UpdateError()} from \codew{UpdateSubProcess.cpp} file
\item Average error calculated from summing up the facets' errors %weighted with facet area
corrected by the number of facets
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item \textit{Placeholder}
%\item Calculate step size in \codew{getStepSize()} (\codew{UpdateMainProcess.cpp}) Was ist der Sinn dahinter?
%\item Multiply covering in \codew{hitbuffer} of main process  analogous to subprocesses in \codew{checkSmallCovering($\cdot$)} (\codew{SimulationLinux.cpp}) Verwirrt erstmal nur!
\end{itemize}
\end{comment}
...
\item{In the correction step, the iteration for the current time step is repeated. In \codew{PerformBounce($\cdot$)} (\codew{SimulationMC.cpp}), the covering values from \codew{currentList} will be read, if\\\codew{sHandle$\rightarrow$currentParticle.flightTime <= 1/2*simHistory$\rightarrow$stepSize}. Otherwise covering values from \codew{predictList} will be read.}
\item{At the end of the correction step, the main process is updated as usual and continuing with the next iteration}
\item{Used only, if \codew{p->usePCMethod} is set to \codew{1} (as opposed to \codew{0}) explicitly via the input file}
\item{If \codew{usePCMethod} is set to \codew{0}:  not using the p.c. method; \codew{1}:  using the p.c. method;}
\end{itemize}
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item $K_{\text{real}/\text{virtual}}$ computed in \codew{GetMoleculesPerTP($\cdot$)} (\codew{Simulationcalc.cpp})
\item Divide covering in \codew{hitbuffer}  if previously multiplied (in case of 'smallcovering')
...
\subsubsection{Calculate \& update statistics (with respect to the points in time) of coverage values}
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Calculate mean and standard deviation of covering over the last \codew{p$\rightarrow$rollingWindowSize} iterations
\item Update statistics in \codew{HistoryList::updateStatistics($\cdot$)} (\codew{SimulationLinux.h})
\item End simulation if \codew{p$\rightarrow$convergenceTarget} is reached by average statistics weighted with area. This is evaluated in \codew{HistoryList::getAverageStatistics($\cdot$)} (\codew{SimulationLinux.h})
\end{itemize}
...
\item Load geometry into \codew{Simulation sHandle} and check if values are valid using \codew{loadAndCheckSHandle()}
	...
	\end{itemize}
\item{Iteration of simulation steps until \codew{p$\rightarrow$maxTimeS} or\\ \codew{p$\rightarrow$convergenceTarget} (\codew{p$\rightarrow$convergenceTime},\codew{p$\rightarrow$stopConverged}) is reached. If \codew{p$\rightarrow$usePCMethod} is not set to \codew{0}, repeat same iteration twice: \smallskip}
	\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
	...
	\item[$\bullet$] Update relevant simulation values using \codew{simHistory$\rightarrow$updateStepSize($\cdot$)},\linebreak[4] \codew{UpdateSticking($\cdot$)}, \codew{CalcTotalOutgassingWorker($\cdot$)}, \codew{UpdateDesorption($\cdot$)}. If using p.c. method: done only for the prediction step. The remain set in the correction step.
	...
	\item[--] Update hitbuffers of subprocesses from \codew{sHandle} using \codew{UpdateMCSubHits($\cdot$)}\\(\codew{UpdateSubProcess.cpp})
	\end{itemize}
	...
	\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
	\item[--] Calculate real, physical covering in main process using $K_{\text{real}/\text{virtual}}$ in \codew{UpdateCovering($\cdot$)}\\(\codew{UpdateMainProcess.cpp}), save in \codew{simHistory}: \codew{predictList}
	\end{itemize}
