...
%\draw [help lines] (0,0) grid (4,4);
	...
	{
		\textbf{Main Process 0}
		\nodepart{second} Broadcast hitbuffer\\covering
	};
	...
	{
		\textbf{Main Process 0}
		\nodepart{second} Update real covering in hitbuffer\\ Update SimulationHistory
	};
...
\item \codew{$coveringPath$}: Path to covering file, contains either covering or coverage per facet, file can be exported from ContaminationFlow Windows, etc., e.g.\ \codew{\textasciitilde/covering.txt}; default: \codew{""}
...
\begin{tikzpicture}[node distance=2cm, font=\scriptsize]
	...
	\node[neuron](load) at (5,-2) {Main process:\\Import loadbuffer\\and broadcast};
	\draw let \p1=(input.east), \p2=(load.west) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[neuron](geom) at (10,-2) {Load geometry \&\\init simHistory};
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item HistoryList and SimulationHistory class
\end{itemize}
