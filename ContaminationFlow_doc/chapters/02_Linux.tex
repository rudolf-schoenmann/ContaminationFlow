...
\item Depending on the how many layers of particles are adsorbed.
...
\item $K_{\text{real}/\text{virtual}}$ computed from \codew{Simulationcalc.cpp} file in \codew{GetMoleculesPerTP($\cdot$)}
\end{itemize}
...
\item Particle density computed from \codew{Simulationcalc.cpp} file in \codew{calcParticleDensity($\cdot$)}
\end{itemize}

\subsubsection{Pressure}
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Calculated from sum over orthogonal velocity, facet area, gas mass and $K_{\text{real}/\text{virtual}}$
\item Pressure computed from \codew{Simulationcalc.cpp} file in \codew{calcPressure($\cdot$)}
\end{itemize}
...
\item Start time computed from \codew{Simulationcalc.cpp} file in \codew{calcStartTime($\cdot$)}
\end{itemize}
...
\code{template <typename T> class HistoryList \{\\
	...
	\hphantom{\enskip}pair<\,vector<double>,vector<\,vector<T>\,>\,> historyList;\\
	\hphantom{\enskip}vector<T> currentList;\\
\};}
...
\item Updated after each iteration in \codew{UpdateParticleDensityAndPressure($\cdot$)},\linebreak[4] \codew{UpdateCovering($\cdot$)}, \codew{UpdateErrorMain($\cdot$)} from \codew{UpdateMainProcess.cpp} file
...
\begin{tikzpicture}[node distance=2cm, font=\footnotesize]
	...
	\node[neuron](preprocess) at (6,-0.2) {Calculate desorption,\\cov. thresh. \& step size;\\multiply cov. if small};
	...
	\node[neuron](particle) at (1,-5.8) {Create particle%\\Add residence time
	};
	\draw let \p1=(exist.south), \p2=(particle.north) in [->, line width=0.04cm] (\p1) -- (\p2)node[midway, right]{false};

	\node[neuron](time3) at (6,-5.8){Determine start time};
	...
	\node[neuron](sojourn) at (6,-9.7) {Add residence time};
...
\item Calculation in \codew{StartFromSource($\cdot$)}  from \codew{SimulationMC.cpp} file
\end{itemize}
...
\item Sojourn time randomly calculated from binding energy, facet temperature and sojourn frequency
\item Calculation in \codew{PerformBounce($\cdot$)}  from \codew{SimulationMC.cpp} file
\end{itemize}
...
\item Check if total error reached target error
\end{itemize}
...
\begin{tikzpicture}[node distance=2cm, font=\footnotesize]
	...
	\node[neuron](K) at (0,-2.6) {Calculate $K_{\text{real}/\text{virtual}}$};
	...
	\node[neuron, minimum width=5cm](update) at (7,-1.7) {Update covering to\\ hitbuffer and simHistory};
	\draw let \p1=(cov.north), \p2=(update.south) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[neuron, minimum width=5cm](statistics) at (7,-0.5) {Update statistics};
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Calculate step size in \codew{UpdateMainProcess.cpp} file in \codew{getStepSize()} depending on, if the target error was reached
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Calculate statistical error per facet and total error analogous to calculation in subprocesses
\item Save error per facet in \codew{simHistory$\rightarrow$errorList}
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Calculate mean and standard deviation of quantity over last \codew{p$\rightarrow$rollingWindowSize} iterations
\item Update statistics in \codew{HistoryList::updateStatistics($\cdot$)} from \codew{SimulationLinux.h} file
\end{itemize}
...
\item Iteration until desired maximum simulation time is reached: \smallskip
	...
	\item[$\bullet$] Update simulation values using \codew{simHistory$\rightarrow$updateStepSize($\cdot$)},\linebreak[4] \codew{UpdateSticking($\cdot$)}, \codew{UpdateDesorption($\cdot$)}, \codew{CalcTotalOutgassingWorker($\cdot$)}
	...
	\item[$\bullet$] Calculate real covering in main process using $K_{\text{real}/\text{virtual}}$ in \codew{UpdateCovering($\cdot$)} from \codew{UpdateMainProcess.cpp}, save in \codew{simHistory}
