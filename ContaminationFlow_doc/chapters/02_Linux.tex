...
\item \codew{$H_{vap}$}: minimum energy used for calculation of desorption; default:\codew{0.8\text{E}-19}
\item \codew{$W_{tr}$}: window width used for calculation of desorption; default:\codew{1}
...
\item \codew{$vipFacets$}: vip facets: alternate facet number and its target error; default:\codew{[]}
\end{itemize}
\newpage
...
\item Multiply covering in \codew{hitbuffer} and \codew{simHistory} of main process before simulation using same factor
\end{itemize}

\subsubsection{Divide small covering after  summation of subprocesses}
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Only if covering was multiplied in subprocesses
\item Divide covering in \codew{hitbuffer} and \codew{simHistory} of main process before simulation using same factor
