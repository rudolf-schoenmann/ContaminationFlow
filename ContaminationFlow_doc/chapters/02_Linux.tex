...
\item \codew{$targetPaticles$}: Minimum number of desorbed particles per iteration; default: \codew{1000}
\item \codew{$targetError$}: Maximum error per iteration; default: \codew{0.001}
\item \codew{$hitRatioLimit$}: Ratio at which hits are ignored, default: \codew{1\text{E}-5}
\item \codew{$Tmin$}: minimum time for step size; default:\codew{1\text{E}-4}
\end{itemize}
\newpage
...
\item Time: simulated time (accumulated time steps) instead of computation time 
\end{itemize}
...
\begin{tikzpicture}[node distance=2cm, font=\footnotesize]
	...
	\node[neuron](preprocess) at (5,-0.2) {Calculate desorption,\\sojourn variables,\\cov. thresh., step size};
	\draw let \p1=(start.east), \p2=(preprocess.west) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[neuron](nbSteps) at (5,-1.9) {Calculate steps for \\desired simulation time};
	\draw let \p1=(preprocess.south), \p2=(nbSteps.north) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[neuron](particle) at (5,-3.5) {Create particle};
	\draw let \p1=(nbSteps.south), \p2=(particle.north) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[condition, aspect=1.5](covthresh) at (5,-5.8){Cov. thresh.\\reached?};
	\draw let \p1=(particle.south), \p2=(covthresh.north) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[neuron](facet) at (5,-8.1) {Find next facet};
	\draw let \p1=(covthresh.south), \p2=(facet.north) in [->, line width=0.04cm] (\p1) -- (\p2) node[midway, right]{false};

	\node[condition, aspect=1.5](time1) at (5,-10.1){flight time>\\step size?};
	\draw let \p1=(facet.south), \p2=(time1.north) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[neuron](sojourn) at (10,-10.1) {Add sojourn time};
	\draw let \p1=(time1.east), \p2=(sojourn.west) in [->, line width=0.04cm] (\p1) -- (\p2) node[midway, above]{false};

	\node[condition, aspect=1.5](time2) at (10,-12.5){flight time>\\step size?};
	\draw let \p1=(sojourn.south), \p2=(time2.north) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[neuron](hit) at (10,-14.8) {Record hit};
	\draw let \p1=(time2.south), \p2=(hit.north) in [->, line width=0.04cm] (\p1) -- (\p2) node[midway, right]{false};

	\node[neuron](absorb) at (5,-12.5) {Record absorb};
	...
	\node[condition, aspect=1.5](steps) at (5,-14.8){No.steps\\reached?};
	\draw let \p1=(absorb.south), \p2=(steps.north) in [->, line width=0.04cm] (\p1) -- (\p2);
	\draw let \p1=(hit.west), \p2=(steps.east) in [->, line width=0.04cm] (\p1) -- (\p2);
	\draw let \p1=(steps.west), \p2=(particle.west) in [->, line width=0.04cm] (\p1) --node[midway, above]{false}(\x1-1cm,\y1)-- (\x1-1cm,\y2) -- (\p2);

	\node[condition, aspect=1.5](target) at (5,-17.5){Target error\\reached?};
	\draw let \p1=(steps.south), \p2=(target.north) in [->, line width=0.04cm] (\p1) -- (\p2) node[midway, right]{true};
	\draw let \p1=(target.west), \p2=(nbSteps.west) in [->, line width=0.04cm] (\p1) --node[midway, above]{false}(\x1-1cm, \y1)--(\x1-2cm,\y1)-- (\x1-2cm,\y2) -- (\p2);

	\node[neuron](hitbuffer) at (10,-5.8) {Update hitbuffer};
	...
	\draw let \p1=(covthresh.east), \p2=(hitbuffer.west) in [->, line width=0.04cm] (\p1) -- (\p2) node[midway, above]{true};

	\node[module](end)  at (12,-0.2) {End simulation\\sub process};
	\draw let \p1=(hitbuffer.north), \p2=(end.west) in [->, line width=0.04cm] ([xshift=-1cm]\p1) --(\x1-1cm,\y2)-- (\p2);
...
\begin{tikzpicture}[node distance=2cm, font=\footnotesize]
	...
	\node[module](start)  at (0,0) {Start update\\main process};
	\node[neuron](error) at (0,-2.5) {Calculate error of\\current iteration};
	\draw let \p1=(start.south), \p2=(error.north) in [->, line width=0.04cm] (\p1) -- (\p2);
	...
	\node[module](end)  at (12,0) {End update\\main process};
...
\begin{itemize}[noitemsep,topsep=0pt]
\item Use \codew{SimulationHistory::currentStep} to calculate logarithmic step size
...
\item Increment \codew{SimulationHistory::currentStep} if step size not decreased and \codew{ProblemDef::targetError} reached
...
\begin{tikzpicture}[node distance=2cm, font=\footnotesize]
	...
	\node[module](preprocess) at (0,-3.7) {Initialization};
	\draw let \p1=(smpi.south), \p2=(preprocess.north) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[neuron](bcast) at (0,-6) {Main process:\\broadcast hitbuffer\\ and $currentStep$};
	...
	\node[neuron, rotate=90, minimum height=1.4cm](sum) at (7.8,-6){Summarize results\\in main hitbuffer};
	...
	\draw let \p1=(sum.south), \p2=(update.west) in [->, line width=0.04cm] (\p1) -- (\p2);

	\node[condition](max) at (11, -3.7) {Max. time\\reached?};
...
\item Iteration until desired maximum simulation time is reached: \smallskip
	\begin{itemize}[noitemsep,topsep=0pt]
	\item[$\bullet$] Reset hitbuffer counters using \codew{initbufftotero($\cdot$)}, broadcast using \codew{MPI\_Bcast($\cdot$)}
	...
	\item[--] Update hitbuffer of sub processes from \codew{sHandle} using \codew{UpdateSubHits($\cdot$)} from \codew{UpdateSubProcess.cpp}
	\end{itemize}
