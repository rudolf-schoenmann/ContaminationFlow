...
\item n: desired number of worker processes; simulation on K=N-1 worker processes
\item MolflowLinux: path to application, e.g.\ \codew{\textasciitilde/MolflowLinux/Debug/MolflowLinux}
\item loadbufferPath: path to loadbuffer file, that contains geometry, e.g.\ \codew{\textasciitilde/loadbuffer}
\item hitbufferPath: path to hitbuffer file, that contains counters, etc., e.g.\ \codew{\textasciitilde/hitbuffer}
...
\item simulationTime (optional): simulation time per iteration step; default: \codew{10.0}
\item unit (optional): simulation time unit; default: \codew{s}
\item maxTime (optional): maximum simulation time; default: \codew{10.0}
\item maxUnit (optional): maximum simulation time unit; default: \codew{y}
...
\item \codew{$E_{de}$}, energy used for calculation of desorption; default:\codew{1\text{E}-21}
...
\item Start iteration: iterationNumber steps of length simulationTime * unit:
	...
	\item[$\bullet$] Update sticking factor and desorption rate in sub processes using \codew{UpdateSticking($\cdot$)} and \codew{UpdateDesorptionRate($\cdot$)} from \codew{UpdateSubProcess.cpp}
...
\subsubsection{Pretesting of time step \codew{$T_{min}$}}
\begin{itemize}[noitemsep,topsep=0pt]
\item Checks whether current time step would cause covering to get negative
\item Adapts time step if needed
\item Pretesting in \codew{UpdateMainProcess.cpp} file in \codew{preTestTimeSteo($\cdot$)}
\end{itemize}
