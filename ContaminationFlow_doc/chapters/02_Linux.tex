...
\item \codew{$maxStepSize$}: maximum time for step size; default:\codew{max}
...
\item \codew{$vipFacets$}: vip facets, alterning facet number and target error; default:\codew{[]}
\end{itemize}
\newpage
...
\begin{tikzpicture}[node distance=2cm, font=\footnotesize]
	...
	\node[neuron](particle) at (1,-5.8) {Create particle};
	\draw let \p1=(exist.south), \p2=(particle.north) in [->, line width=0.04cm] (\p1) -- (\p2)node[midway, right]{false};

	\node[condition, aspect=1.5](covthresh) at (6,-5.8){Cov. thresh.\\reached?};
	\draw let \p1=(particle.east), \p2=(covthresh.west) in [->, line width=0.04cm] (\p1) -- (\p2);
	\draw let \p1=(exist.east), \p2=(covthresh.north) in [->, line width=0.04cm] (\p1) --(\x1+1cm,\y1)node[midway, above]{true}--(\x2,\y1)-- (\p2);

	\node[neuron](facet) at (1,-7.7) {Find next facet};
	...
	\draw let \p1=(steps.west), \p2=(particle.west) in [->, line width=0.04cm] (\p1) --(\x1-0.5cm,\y1)-- node[midway, right]{false}(\x1-0.5cm,\y1+1cm)-- (\x1-0.5cm,\y2) -- (\p2);
	...
	\node[neuron](hitbuffer) at (11,-5.8) {Update hitbuffer};
...
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Multiply covering to reach \codew{ProblemDef::coveringThreshMin}
\item Adapt covering threshold and \codew{simHistory$\rightarrow$coveringList}
...
\item Check if normal facets combined reached target error
\item Neglect normal facets that reached \codew{ProblemDef::hitRatioLimit}
...
\subsubsection{Divide small covering}
\begin{itemize}[noitemsep,topsep=0pt, partopsep=0pt]
\item Divide covering if multiplied this iteration
\item Adapt hitbuffer and \codew{simHistory$\rightarrow$coveringList}
\item Calculation in \codew{UndoSmallCovering($\cdot$)} from \codew{SimulationLinux.cpp}
\end{itemize}
